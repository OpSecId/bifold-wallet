...
import { AttestationEventTypes } from '../../../../packages/core/src/types/attestation'
import { DeliveryStackParams } from '../../../../packages/core/src/types/navigators'
import { useAppAgent } from '../../../../packages/core/src/utils/agent'
import { HistoryCardType, HistoryRecord } from '../../../../packages/core/src/modules/history/types'
import Toast from 'react-native-toast-message'

import { Processing, RiseIn } from '../components/ui'
import ProofRequest from './Proof'
import CredentialOffer from './Offer'
...
const Connection: React.FC<ConnectionProps> = ({ navigation, route }) => {
  const { oobRecordId, proofId, credentialId } = route.params
  const [
    logger,
    { useNotifications },
  ...
  const processingCopy = () => {
    if (state.shouldShowProofComponent) {
      return { title: 'Opening the request', detail: 'Preparing what they asked you to share.' }
    }
    if (state.shouldShowOfferComponent) {
      return { title: 'Incoming credential', detail: 'The offer is landing in this wallet.' }
    }
    return { title: 'Connecting', detail: 'Waiting on the other agent. This can take a moment.' }
  }
  ...
  const displayComponent = () => {
    if (state.inProgress || state.attestationLoading) {
      ...
      return (
        <Processing
          title={copy.title}
          detail={copy.detail}
          onCancel={onDismissModalTouched}
        />
      )
    }
    ...
    if (state.shouldShowProofComponent) {
      return (
        <RiseIn>
          <ProofRequest proofId={proofId ?? state.notificationRecord.id} navigation={navigation} />
        </RiseIn>
      )
    }
    ...
    if (state.shouldShowOfferComponent) {
      return (
        <RiseIn>
          <CredentialOffer credentialId={credentialId ?? state.notificationRecord.id} navigation={navigation} />
        </RiseIn>
      )
    }
  }
